...
export const scrape = async (profileQuery) => {
    ...
    var public_repos;
    try {
        public_repos = await page.$eval("#js - pjax - container > div.mt - 4.position - sticky.top - 0.d - none.d - md - block.color - bg -default.width - full.border - bottom.color - border - muted > div > div > div.Layout - main > div > nav > a: nth - child(2) > span", el => el.innerHTML)
    } catch (err) {
        public_repos = 0
    }
    ...
    // save data here and use data from mongo
